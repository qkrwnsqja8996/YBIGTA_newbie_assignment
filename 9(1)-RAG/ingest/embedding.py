...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    for i in tqdm(range(0, len(texts), BATCH_SIZE), desc="Embedding (Sequential)"):
        batch_texts = texts[i : i + BATCH_SIZE]
        
        batch_embs = _embed_batch_safe(client, batch_texts)
        total_embeddings.extend(batch_embs)
        
    embedding_array = np.array(total_embeddings, dtype=np.float32)
