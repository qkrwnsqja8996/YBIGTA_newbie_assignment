...
My Prompting은 CoT의 논리적 사고 흐름을 계승하지만 전문가적 페르소나와 검증(단계를 도입하여 추론의 완결성을 한층 높인 방식이다. 모델에게 '세계적인 수학 대회 코치'라는 구체적인 역할을 부여하여 문제에 더욱 엄격하고 신중하게 접근하도록 유도하였으며, 답안 제출 전 반드시 계산 과정을 재확인(Double-check)하는 절차를 강제하여 우발적인 연산 오류를 스스로 방지하도록 설계하였다. 이러한 전략은 모델이 단순한 문제 풀이를 넘어 전문가 수준의 책임감을 갖게 함으로써 논리적 정합성을 극대화했고, 결과적으로 3-shot 기준 86%라는 가장 높은 정답률을 달성하여 기존 CoT 방식보다 더욱 신뢰할 수 있는 성능을 입증하였다.