...
def connect_with_fallback(ips: list[str], port: int, timeout: float, prefer: str = "any") -> TCPConnectResult:
    ...
    ordered = []
    if prefer == "ipv4":
        ordered = v4_list + v6_list
    elif prefer == "ipv6":
        ordered = v6_list + v4_list
    else:
        ordered = ips
    ...
    for ip in ordered:
        ...
        sock, ms, err = connect_one(ip, port, timeout) # TODO: 로직 구현
        
        if sock is not None:
            try:
                l_addr = sock.getsockname()
                p_addr = sock.getpeername()
            except Exception:
                l_addr = None
                p_addr = None
    ...
    return TCPConnectResult(
        ...
        local_addr=None,
        peer_addr=None,
        error=last_err or "모든 IP에 대해 연결 실패",
        sock=None
    )        